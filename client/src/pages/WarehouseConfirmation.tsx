...
export default function WarehouseConfirmationPage() {
  ...
  const handleQuickConfirm = async (tx: any) => {
    ...
    try {
      ...
      setConfirmStatus({
        kind: "success",
        message: "Xác nhận lô hàng thành công!",
      });
  ...
  const handleDetailedConfirm = async () => {
    ...
    if (scannedImeis.size !== expectedImeis.size) {
      setConfirmStatus({
        kind: "error",
        message: `Số lượng IMEI quét (${scannedImeis.size}) không khớp với lô hàng (${expectedImeis.size}).`,
      });
      return;
    }
    ...
    if (mismatch) {
      setConfirmStatus({
        kind: "error",
        message: "Có IMEI không thuộc lô hàng này.",
      });
      return;
    }
  ...
  const handleFileUpload = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      const lines = text
        ...
        .map((l) => l.trim())
        .filter(Boolean);
      const parsed: ScannedRow[] = lines
        .map((line) => {
          const parts = line.split(",");
          if (parts.length >= 1)
            return { id: crypto.randomUUID(), imei: parts[0] };
          return null;
        })
              ...
              {!warehouseId ? (
                <div className="flex flex-col items-center justify-center min-h-[50vh]">
                  ...
                  <p className="text-slate-500 mt-2">
                    Vui lòng chọn một kho để xem hàng đang đến.
                  </p>
                </div>
              ) : (
                  ...
                  <div className="bg-white border border-border-soft rounded-2xl p-6 lg:p-8 relative shadow-apple-md">
                    <div className="flex items-center justify-between mb-6">
                      <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                        ...
                        Hàng Đang Đến
                        {transfers.length > 0 && (
                          <span className="ml-2 bg-lilac-200 text-lilac-800 text-xs px-2 py-0.5 rounded-full">
                            {transfers.length}
                          </span>
                        )}
                      </h2>
                      <button
                        ...
                        className="text-sm font-semibold text-lilac-500 hover:text-lilac-600 transition-colors"
                      >
                        Tải Lại
                      </button>
                    </div>
                    ...
                    {loading ? (
                      ...
                      <div className="text-center py-16 px-4 border-2 border-dashed border-lilac-100 rounded-3xl bg-white/50">
                        ...
                        <p className="text-slate-500 text-sm mt-1">
                          Kho {currentWarehouse?.name} hiện không có lô hàng nào
                          đang chờ xác nhận.
                        </p>
                      </div>
                    ) : (
                            ...
                            >
                              ...
                              <div className="space-y-1 mb-4">
                                ...
                                <div className="text-[13px] text-slate-500 flex justify-between pt-1 border-t border-slate-100">
                                  <span>Số lượng:</span>
                                  <span className="font-bold text-lilac-600">
                                    {totalItems} máy
                                  </span>
                                </div>
                              </div>
                              ...
                              {isSelected && (
                                <div className="mt-4 pt-4 border-t border-black/5 space-y-3">
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      handleQuickConfirm(tx);
                                    }}
                                    disabled={
                                      confirmStatus.kind === "submitting"
                                    }
                      ...
                      >
                        <div className="flex justify-between items-center mb-6">
                          <h3 className="text-lg font-bold text-slate-800">
                            Xác nhận Chi tiết bằng IMEI (Bảo mật quét mã)
                          </h3>
                          <button
                            onClick={() => {
                              setSelectedTx(null);
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-4">
                            ...
                            >
                              ...
                              <span className="text-sm font-bold text-lilac-600">
                                Tải lên file CSV kiểm đếm
                              </span>
                            </div>
                            <p className="text-xs text-slate-500 px-2 leading-relaxed">
                              <strong>Mẹo:</strong> Upload file CSV chứa danh
                              sách IMEI đã quét thực tế tại kho để hệ thống tự
                              động đối chiếu số lượng và mã máy gửi đến.
                            </p>
                          </div>
                          ...
                          <div className="flex flex-col justify-between">
                            <div className="bg-white rounded-2xl p-4 border border-border-soft h-32 overflow-y-auto shadow-apple-inset">
                              ...
                              {scannedRows.slice(0, 3).map((r) => (
                                <div
                                  key={r.id}
                                  className="text-xs text-slate-600 font-mono py-1 border-b border-slate-100 last:border-0 truncate"
                                >
                                  {r.imei}
                                </div>
                              ))}
                              {scannedRows.length > 3 && (
                                <div className="text-xs text-slate-400 mt-1 italic font-medium">
                                  ...và {scannedRows.length - 3} thiết bị khác
                                </div>
                              )}
                            ...
                            <div className="mt-4">
                              {confirmStatus.kind === "error" && (
                                <div className="text-red-500 text-sm mb-2 px-2 flex items-center gap-1.5">
                                  <AlertCircle className="w-4 h-4 shrink-0" />
                                  {confirmStatus.message}
                                </div>
                              )}
                              {confirmStatus.kind === "success" && (
                                <div className="text-emerald-500 text-sm mb-2 px-2 flex items-center gap-1.5">
                              ...
                              <button
                                ...
                                className={cn(
                                  ...
                                  scannedRows.length > 0
                                    ? "bg-violet-600 text-white shadow-apple-md hover:opacity-95"
                                    : "bg-slate-100 text-slate-400 shadow-flat",
                                )}
                              >
                                {confirmStatus.kind === "submitting" ? (
